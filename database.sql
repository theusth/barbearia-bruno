-- ==========================================================
-- BRUNO BARBEARIA - SUPABASE / POSTGRESQL
...
-- Apenas um agendamento ativo por data + horário.
...
-- 2) CRIAR PERFIL AUTOMATICAMENTE AO CADASTRAR NO AUTH
...
-- 3) FUNÇÃO SEGURA PARA VERIFICAR SE É PROPRIETÁRIO
...
-- 6) CONSULTAR APENAS HORÁRIOS OCUPADOS (SEM EXPOR DADOS DE CLIENTES)
...
-- 7) CANCELAMENTO PELO PRÓPRIO CLIENTE
...
-- 8) ALTERAR STATUS PELO PROPRIETÁRIO
...
-- 9) PROMOVER O DONO PELO SQL EDITOR
-- Segurança: esta função NÃO pode ser executada pelo navegador.
...
begin
  ...
  if target_id is null then
    raise exception 'Usuário com esse e-mail não encontrado. Crie a conta primeiro.';
  end if;
...
-- 10) SERVIÇOS INICIAIS
insert into public.services (name, description, price, duration_minutes, active, sort_order)
select * from (values
  ('Corte Premium', 'Corte personalizado com acabamento, pezinho e finalização.', 45.00::numeric, 45, true, 1),
  ('Barba Completa', 'Modelagem da barba e acabamento preciso.', 35.00::numeric, 35, true, 2),
  ('Corte + Barba', 'Experiência completa para alinhar cabelo e barba.', 70.00::numeric, 70, true, 3),
  ('Pezinho / Acabamento', 'Acabamento rápido para manter o visual alinhado.', 20.00::numeric, 20, true, 4)
) as seed(name, description, price, duration_minutes, active, sort_order)
where not exists (select 1 from public.services);

-- ==========================================================
-- DEPOIS DE CRIAR A CONTA DO BRUNO PELO SITE:
--
-- select public.promote_owner('SEU_EMAIL_AQUI');
--
-- Rode a linha acima NO SQL EDITOR substituindo pelo e-mail da conta.
-- Depois saia e entre novamente no site.
-- ==========================================================
...
-- ==========================================================
-- ATUALIZAÇÃO - BLOQUEIO DE HORÁRIOS / CLIENTES FIXOS
-- Rode este arquivo UMA VEZ no Supabase > SQL Editor.
-- NÃO apaga os agendamentos existentes.
-- ==========================================================
...
-- 2) IMPEDE BLOQUEAR UM HORÁRIO QUE JÁ POSSUI AGENDAMENTO
...
begin
  if exists (
    select 1
    from public.appointments a
    where a.appointment_date = new.block_date
      and a.appointment_time = new.block_time
      and a.status in ('confirmed','completed')
  ) then
...
-- 3) IMPEDE CLIENTE DE AGENDAR UM HORÁRIO BLOQUEADO
...
begin
  if new.status = 'confirmed' and exists (
    ...
    where b.block_date = new.appointment_date
      and b.block_time = new.appointment_time
  ) then
...
-- 4) DISPONIBILIDADE: RETORNA AGENDAMENTOS + BLOQUEIOS SEM EXPOR DADOS
create or replace function public.get_booked_times(p_date date)
returns table (appointment_time time)
language sql
stable
...
-- 5) LOGS DE BLOQUEIO E DESBLOQUEIO
...
begin
  if tg_op = 'INSERT' then
    perform public.write_activity_log(
      ...
      'blocked_slot',
      new.id::text,
      'Horário bloqueado para ' || new.reserved_for || ' • ' ||
      to_char(new.block_date, 'DD/MM/YYYY') || ' às ' ||
      to_char(new.block_time, 'HH24:MI'),
      jsonb_build_object(
        'date', new.block_date,
        'time', new.block_time,
    ...
    perform public.write_activity_log(
      ...
      'blocked_slot',
      old.id::text,
      'Horário liberado: ' || old.reserved_for || ' • ' ||
      to_char(old.block_date, 'DD/MM/YYYY') || ' às ' ||
      to_char(old.block_time, 'HH24:MI'),
      jsonb_build_object(
        'date', old.block_date,
        'time', old.block_time,
        'reserved_for', old.reserved_for
      )
    );
    return old;
  end if;
...
drop trigger if exists trg_log_blocked_slot_change on public.blocked_slots;
create trigger trg_log_blocked_slot_change
after insert or delete on public.blocked_slots
